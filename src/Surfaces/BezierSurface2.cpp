...
bf::vec3d bf::BezierSurface2::parameterHesseUU(double uf, double vf) const {
	auto &&[iu, iv, u, v] = setParameters(uf, vf);
	bf::vec4d uvec = {0., 0., 2. , 6. * u};
	bf::vec4d vvec = {1.,v,v*v,v*v*v};
	bf::vec4d pl = uvec * M6 / 6.;
	bf::vec4d pr = transpose(M6) * vvec / 6.;
	bf::vec3d P[16];
...
bf::vec3d bf::BezierSurface2::parameterHesseUV(double uf, double vf) const {
	auto &&[iu, iv, u, v] = setParameters(uf, vf);
	bf::vec4d uvec = {0., 1., 2. * u, 3. * u * u};
	bf::vec4d vvec = {0., 1., 2. * v, 3. * v * v};
	bf::vec4d pl = uvec * M6 / 6.;
...
bf::vec3d bf::BezierSurface2::parameterHesseVV(double uf, double vf) const {
	auto &&[iu, iv, u, v] = setParameters(uf, vf);
	bf::vec4d uvec = {1.,u,u*u,u*u*u};
	bf::vec4d vvec = {0., 0., 2. , 6. * v};
	bf::vec4d pl = uvec * M6 / 6.;
	bf::vec4d pr = transpose(M6) * vvec / 6.;
	bf::vec3d P[16];
	for (int i = 0; i < 16; i++) {
